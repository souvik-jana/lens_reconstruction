"""
EM+GW tutorial: fisher-source, deriv-approx-source, nautilus-source.
...
Y0_LO = float(src[0]) - src_hw
Y0_HI = float(src[0]) + src_hw
...
if RUN_FISHER_SOURCE:
    ...
    plot_source_posterior(
        ...
        cfg={
            ...
            "plot": {
                "plot_mode": "groupwise",
                "save_path": "source_posterior_{group_name}.png",
            },
        },
    )
...
if RUN_NAUTILUS_SOURCE:
    ...
    print(f"\n--- EM+GW: nautilus-source (resume={NAUTILUS_RESUME}) ---\n")
    samples, truths = run_inference(
        ctx,
        mode="EM+GW",
