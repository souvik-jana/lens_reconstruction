"""
Pooled copy of tutorial_gw_only.py: identical config and methods, with the run
section under ``if __name__ == "__main__":`` so cfg["nautilus"]["pool"] can be used.

Pool workers are fresh processes that re-import this file. Without the guard each
worker would re-run the whole tutorial -- re-simulating, re-fitting Fisher and
deriv-approx -- and then try to start a pool of its own, which Python refuses with
"An attempt has been made to start a new process before the current process has
finished its bootstrapping phase". With the guard, importing this file only defines
CFG and the helpers, and each worker rebuilds just the likelihood from the ctx it is
handed (~2.5 s).

POOL = None runs serially, in which case this file behaves exactly like
tutorial_gw_only.py. Only nautilus-source uses the pool; fisher-source and
deriv-approx-source always run once, in this process.
...
POOL = 4  # nautilus worker processes; None = serial
...
# Simulated lens ellipticity, written as q/phi and converted to the e1/e2 the
# simulation kwargs actually carry (herculens convention, phi in radians).
# make_default_cfg hands out the module-level DEFAULT_KWARGS_LENS by reference, so
# copy before mutating or every later cfg in this process inherits the change.
LENS_Q = 0.5
LENS_PHI_DEG = 30.0
...
print(f"Lens ellipticity: q={LENS_Q}, phi={LENS_PHI_DEG} deg "
      f"-> e1={_e1:.6f}, e2={_e2:.6f}")
...
CFG["gw"]["solver_params"]["jaxtronomy"]["solver"] = "analytical"#"lenstronomy"#"analytical"#"lenstronomy"

# CFG["gw"]["solver_params"]["backend"] = "helens"
# CFG["gw"]["solver_params"]["helens"]["nsubdivisions"] = 5   # raise if images are missed
# CFG["gw"]["solver_params"]["helens"]["niter"] = 8
# CFG["gw"]["solver_params"]["helens"]["pixel_scale_factor"] = 0.2
...
if __name__ == "__main__":
    ...
    Y0_LO, Y0_HI = -0.001, 0.05#-0.6, 0.6#0.018, 0.022#0.01992, 0.02005
    Y1_LO, Y1_HI = -0.004, 0.0018#-0.6, 0.6#0.004, 0.016#0.0091, 0.0106
    ...
    PRIORS = {
        ...
        "T_star": dist.Uniform(2e6, 1.5e7),#float(truth_params["T_star"]),  # dist.Uniform(1e-1, 1e12),
        # "T_star": dist.TruncatedNormal(float(truth_params["T_star"]), 2.2e+07, low=1e-3, high=3.5e7),
        "dL": dist.Uniform(10000, 24000.0),#float(truth_params["dL"]), #dist.Uniform(1e-5, 50000.0),
        ...
        "lens0_gamma": dist.Uniform(1.5, 3.0), #float(truth_params["lens0_gamma"]),#dist.Uniform(1.5, 2.4), #float(truth_params["lens0_gamma"]),
        "y0gw": dist.Uniform(Y0_LO, Y0_HI),
        "y1gw": dist.Uniform(Y1_LO, Y1_HI),
    }
    ctx["cfg"]["priors"] = dict(PRIORS)

    # nautilus-source only: used if y0gw/y1gw are absent from cfg["priors"].
    # If this block is also omitted → package default Uniform(-1, 1).
